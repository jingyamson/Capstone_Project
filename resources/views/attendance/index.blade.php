...
function checkAttendance(type, day, attendanceDate, status) {
    ...
    .then(data => {
        ...
        if (data.success) {
            // Redirect to the correct attendance page, keeping filters in the URL
            const redirectUrl = '{{ route('attendance.index', ['student_id' => $student->id, 'subject_id' => $subjectId, 'type' => ':type']) }}'.replace(':type', attendanceType);
